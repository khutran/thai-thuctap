 ...
 * @since  2.3.0
 */
class IG_Pb_Converter_Site_Origin extends IG_Pb_Converter {
	/**
	...
	protected $mapping = array(
		...
		'ig_testimonial' => array(),

		'ig_item_testimonial' => array(
			'attributes' => array(
				'name'  => 'name',
				'image' => 'image_file',
			),
	...
	public function __construct( $post ) {
		...
		add_filter( "ig_pb_parse_data_{$this->converter}", array( &$this, 'parse_site_origin_data' ) );
	...
	/**
	 * Parse Site Origin data from given post then convert to IG PageBuilder data.
	 *
	...
	protected function parse_siteorigin_panels_widget_testimonial( $widget, $class = '' ) {
		...
		if ( ! array_key_exists( 'ig_testimonial', $shortcode_tags ) ) {
			// Equivalent element not available, embed original widget directly
			return $this->parse_widget( $widget, $class );
		}
