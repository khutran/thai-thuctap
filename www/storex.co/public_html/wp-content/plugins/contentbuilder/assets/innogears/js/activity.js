...
(function($) {
	$.IG_Activity_Log = {
		...
		/**
		 * Initialize IG Activity Log.
		 * 
		 * @return  void
		 */
		init: function() {
			var	self = this,
				toolBar = $('#ig_page_builder .jsn-form-bar'),
				tab_switcher = $('a[href="#ig_editor_tab2"]');

			// Create Undo/Redo buttons
			self.buttons = {
				undo: $('<button id="ig-pb-activity-undo" title="Undo" class="btn btn-default" type="button" disabled="disabled" />').append('<i class="icon-undo" />').click(function(event) {
				...
				redo: $('<button id="ig-pb-activity-redo" title="Redo" class="btn btn-default" type="button" disabled="disabled" />').append('<i class="icon-redo" />').click(function(event) {
					event.preventDefault();
			...
			toolBar.after(
				$('<div id="ig-pb-activity" style="position:relative" />').append(
			...
			var positionButtons = function(event) {
				if ((!event || event.target == window || event.target == document) && tab_switcher.parent().hasClass('active')) {
					var scrollTop = $(window).scrollTop(), toolTop = $('#ig-pb-activity').offset().top, spacing = $('#wpadminbar').outerHeight(true);

					if (toolTop - scrollTop < spacing) {
						$('#ig-pb-activity').children().css({
							position: 'fixed',
							top: spacing + 'px',
							left: '50%',
							'margin-left': '-' + ($('#ig-pb-activity').children().width() / 2) + 'px',
							'z-index': 100000,
						}).show();
					} else {
						$('#ig-pb-activity').children().css({
			...
			// Track add action
			$(document).on('ig_pb_after_add_element', function(event, element, extra) {
				self.save('add', element, extra);
			});

			// Track edit action
			$(document).on('ig_pb_after_edit_element', function(event, element, before) {
				self.save('edit', element, before);
			});

			// Track delete action
			$(document).on('ig_pb_before_delete_element', function(event, element) {
		...
		save: function(type, after, before) {
			...
			after.each(function(index, element) {
				...
				$('#ig_page_builder .jsn-row-container').each(function(i, e) {
					if (row > -1) {
						return;
					}
		...
		replace: function(from, to) {
			...
			$.each(from, function(i, e) {
				...
				var	row_index = e.position ? e.position.row : to[i].position.row,
					row = $('#ig_page_builder .jsn-row-container').eq(row_index),
				...
				if (e.html.match(/jsn-row-container/) || to[i].html.match(/jsn-row-container/)) {
					...
					// Re-get row
					affected_element = $('#ig_page_builder .jsn-row-container').eq(e.position ? e.position.row : to[i].position.row);
					...
					if (countColumn) {
						$.IG_PB_Layout.initResizable(countColumn);
						$.IG_PB_Layout.rebuildSortable();
					}
				...
				$.IG_PB_Layout.updateSpanWidthPBDL($.IG_PB_Layout, $.IG_PB_Layout.wrapper, $.IG_PB_Layout.wrapper.width());
			...
			return $('#ig_page_builder .jsn-row-container').eq(affected_row);
		},
		...
		redo_add: function(data) {
			...
			$.each(data.after, function(i, e) {
				if (e.html.match(/jsn-row-container/)) {
					$.IG_PB_Layout._addRow(
						$.IG_PB_Layout.wrapper,
						null,
						e.html,
						e.position
					);
					...
					// Search element based on saved position
					var	row = $('#ig_page_builder .jsn-row-container').eq(e.position.row).removeClass('active-shortcode'),
					...
					// Search element based on saved position
					var	row = $('#ig_page_builder .jsn-row-container').eq(e.position.row),
	...
	// Schedule to init IG Activity Log on window load event
	$(window).load(function() {
		$.IG_Activity_Log.init();
	});
})(jQuery);
