...
(function ($) {
	...
	JSNLayoutCustomizer.prototype = {
			...
			$(".ig-row-content").sortable({
				...
				stop: $.proxy(function(event, ui) {
					...
					if (ui.item[0].__changed) {
						$(document).trigger('ig_pb_after_edit_element', [ui.item.closest('.jsn-row-container'), ui.item[0].__before]);
					}
				...
				$(e).resizable({
					...
					stop: $.proxy(function(event, ui) {
						...
						if (ui.element[0].__class.replace(' ui-resizable-resizing', '') != ui.element.attr('class')) {
							$(document).trigger('ig_pb_after_edit_element', [$([ui.element.parent()[0], ui.element[0].__next.parent()[0]]), ui.element[0].__before]);
						}
		...
		addRow: function(self, this_wrapper){
			this.wrapper.delegate('#jsn-add-container', 'click', function(event) {
				event.preventDefault();

				self._addRow(this_wrapper, this);
			});
		},

		_addRow: function(this_wrapper, target, row_html, position) {
			var self = this;
			...
			if (row_html && position) {
				...
				row_html = $(ig_pb_remove_placeholder($('#tmpl-ig_row').html(), 'custom_style', 'style="display:none"'));

				$(target).before(row_html);

				// Trigger an event after adding an element
				$(document).trigger('ig_pb_after_add_element', row_html);
		...
		swapContentRow: function(a, b, direction) {
			...
			if (direction > 0) {
				...
				b.animate({top: '' + (stylea.height + extra2) + 'px'}, time, effect, function() {
					...
					$(document).trigger('ig_pb_after_edit_element', [$([b[0], a[0]]), before]);
				});
			} else {
		...
		_addColumn: function(column_html, target, position) {
				...
				for (var i = 0; i < rows.length; i++) {
					if (i == position.row) {
						...
						if (columns.length) {
							...
							position = parentForm.find('.ig-row-content');
						}
			...
			if (countColumn < 12) {
				...
				// Add new column
				var rowContent = parentForm.find(".ig-row-content");
				...
				if (typeof position != 'undefined') {
					...
					// Trigger an event after adding an element
					$(document).trigger('ig_pb_after_add_element', column_html);
				}
		...
		sortableElement: function() {
			...
			$(".jsn-element-container").sortable({
				...
				stop: function(event, ui) {
					// Trigger an event after sorting elements
					if (ui.item[0].__changed) {
						$(document).trigger('ig_pb_after_edit_element', [$(ui.item[0].__after), ui.item[0].__before]);
					}
		...
		addElement:function(){
			...
			this.wrapper.delegate(this.addelements,"click",function(e){
				self.showPopover(box, e, self, $(this), function(){
					$.HandleCommon.setFilterFields('#ig-add-element');
		...
		searchElement:function(){
			var self = this;
			$.HandleCommon.setQuickSearchFields('#ig-add-element');
		},
	...
	$.HandleCommon.removeConfirmMsg = function(item, type, column_to_row, callback, silent) {
		...
		if (confirm_) {
			if (!silent) {
				...
				$(document).trigger('ig_pb_before_delete_element', item);
			}
	...
	$(document).ready(function() {
		...
		// Trigger when TinyMCE is changed
		setTimeout(function () {
			if(tinymce.editors.length){
				tinymce.editors[0].onChange.add(function (ed, e) {
					// Update HTML view textarea (that is the one used to send the data to server).
					ed.save();

					_change_editor = 1;
					text_content = ed.getContent();
				});
			}
		}, 1000);
		...
		$('#ig_editor_tabs a').click(function (e, init_layout) {
			...
			// Init IG PageBuilder
			if(this.hash == '#ig_editor_tab2' && ! $.IG_PB_Layout){
				$.IG_PB_Layout = new JSNLayoutCustomizer();
				$.IG_PB_Layout.init($(".ig-pb-form-container.jsn-layout .jsn-row-container"));
			}
			...
			switch (hash) {
				...
				case '#ig_editor_tab2':
					...
					if(_change_editor){
						...
						$('.ig-pb-form-container.jsn-layout #ig-select-media').nextAll( ".jsn-item" ).remove();
						...
						$.HandleElement.updatePageBuilder(tab_content, function(){
							...
							$.IG_PB_Layout.fnReset($.IG_PB_Layout, true);
							$.IG_PB_Layout.moveItemDisable($.IG_PB_Layout.wrapper);
						});
