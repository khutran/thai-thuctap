...
if ( ! class_exists( 'IG_Product_Addons' ) ) :
 ...
 * @since    1.0.0
 */
class IG_Product_Addons extends IG_Product_Info {
	/**
	...
	public static function manage() {
		...
		try {
			...
			if ( empty( $addon ) ) {
				throw new Exception( __( 'Missing addon to be installed.', IG_LIBRARY_TEXTDOMAIN ) );
			}
			...
			if ( empty( $action ) || ! array_key_exists( $action, self::$actions ) ) {
				throw new Exception( __( 'The requested action is invalid.', IG_LIBRARY_TEXTDOMAIN ) );
			}
	...
	protected static function execute( $addon, $action ) {
		...
		if ( 'update' == $action || 'uninstall' == $action ) {
			...
			if ( empty( $plugin ) ) {
				throw new Exception(
					__( 'update' == $action ? 'Cannot detect plugin to be updated.' : 'Cannot detect plugin to be removed.', IG_LIBRARY_TEXTDOMAIN )
				);
			}
		}
			...
			if ( 'authenticate' != $action ) {
				...
				if ( isset( $_GET['core'] ) && ( $core = self::get( $_GET['core'] ) ) ) {
					...
					if ( $core_version && version_compare( $core_version, $core['Version'], '>' ) ) {
						...
						throw new Exception(
							sprintf(
								__( "Cannot install %1\$s v%2\$s.\nThis version requires %3\$s v%4\$s while you are using %5\$s v%6\$s.", IG_LIBRARY_TEXTDOMAIN ),
	...
	protected static function download( $addon, $authentication = false, $username = null, $password = null, $authenticate_only = false ) {
		...
		if ( $wp_filesystem->size( $result ) < 10 ) {
			...
			switch ( $content ) {
				case 'ERR00':
					throw new Exception( __( 'Invalid Parameters! Can not verify your product information.', IG_LIBRARY_TEXTDOMAIN ) );
				break;

				case 'ERR01':
					throw new Exception( __( 'Invalid username or password.', IG_LIBRARY_TEXTDOMAIN ) );
				break;

				case 'ERR02':
					throw new Exception( __( 'We could not find the product in your order list. Seems like you did not purchase it yet.', IG_LIBRARY_TEXTDOMAIN ) );
				break;

				case 'ERR03':
					throw new Exception( __( 'Requested file is not found on server.', IG_LIBRARY_TEXTDOMAIN ) );
				break;

				default:
					if ( 'true' != $content && 'yes' != $content && '1' != $content ) {
						throw new Exception( __( 'Cannot connect to InnoGears.com server.', IG_LIBRARY_TEXTDOMAIN ) );
					}
				break;
			}
		}
		...
		if ( ! $authenticate_only ) {
			if ( ! $wp_filesystem->copy( $result, $target, true, FS_CHMOD_FILE ) ) {
				...
				if ( ! $wp_filesystem->copy( $result, $target, true, FS_CHMOD_FILE ) ) {
					throw new Exception( __( 'Cannot store add-on package to local file system.', IG_LIBRARY_TEXTDOMAIN ) );
				}
			}
		}
	...
	public static function hook() {
		...
		if ( ! isset( $registered ) ) {
			add_action( 'wp_ajax_ig-addons-management',  array( __CLASS__, 'manage'  ) );
