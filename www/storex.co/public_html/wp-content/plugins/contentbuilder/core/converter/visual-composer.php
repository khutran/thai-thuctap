 ...
 * @since  2.3.0
 */
class IG_Pb_Converter_Visual_Composer extends IG_Pb_Converter {
	/**
	...
	protected $mapping = array(
		...
		'ig_item_carousel' => array(
			'attributes' => array(
				'image_file' => 'image_file',
			),
		),

		'ig_contentclips' => array(
			'attributes' => array(
				'title'                    => 'el_title',
				'size'                     => 'ig_cl_limit',
				'order_by'                 => 'ig_cl_orderby',
				'order'                    => 'ig_cl_order',
				'post_type'                => 'ig_cl_source',
	...
	public function __construct( $post ) {
		...
		add_filter( "ig_pb_parse_data_{$this->converter}", array( &$this, 'prepare_visual_composer_data' ) );

		// Register filters to convert Visual Composer shortcodes
		add_filter( 'ig_pb_convert_vc_row_shortcode'            , array( &$this, 'convert_vc_row_shortcode'          ) );
		add_filter( 'ig_pb_convert_vc_column_shortcode'         , array( &$this, 'convert_vc_column_shortcode'       ) );
		add_filter( 'ig_pb_convert_vc_separator_shortcode'      , array( &$this, 'convert_vc_separator_shortcode'    ) );
		add_filter( 'ig_pb_convert_vc_text_separator_shortcode' , array( &$this, 'convert_vc_separator_shortcode'    ) );
		add_filter( 'ig_pb_convert_vc_single_image_shortcode'   , array( &$this, 'convert_vc_single_image_shortcode' ) );
		add_filter( 'ig_pb_convert_vc_gallery_shortcode'        , array( &$this, 'convert_vc_gallery_shortcode'      ) );
		add_filter( 'ig_pb_convert_vc_images_carousel_shortcode', array( &$this, 'convert_vc_gallery_shortcode'      ) );
		add_filter( 'ig_pb_convert_vc_tabs_shortcode'           , array( &$this, 'convert_vc_tabs_shortcode'         ) );
		add_filter( 'ig_pb_convert_vc_tour_shortcode'           , array( &$this, 'convert_vc_tabs_shortcode'         ) );
		add_filter( 'ig_pb_convert_vc_posts_grid_shortcode'     , array( &$this, 'convert_vc_posts_grid_shortcode'   ) );
		add_filter( 'ig_pb_convert_vc_carousel_shortcode'       , array( &$this, 'convert_vc_posts_grid_shortcode'   ) );
		add_filter( 'ig_pb_convert_vc_posts_slider_shortcode'   , array( &$this, 'convert_vc_posts_grid_shortcode'   ) );
		add_filter( 'ig_pb_convert_vc_button_shortcode'         , array( &$this, 'convert_vc_button_shortcode'       ) );
		add_filter( 'ig_pb_convert_vc_button2_shortcode'        , array( &$this, 'convert_vc_button_shortcode'       ) );
		add_filter( 'ig_pb_convert_vc_cta_button_shortcode'     , array( &$this, 'convert_vc_cta_button_shortcode'   ) );
		add_filter( 'ig_pb_convert_vc_cta_button2_shortcode'    , array( &$this, 'convert_vc_cta_button_shortcode'   ) );
		add_filter( 'ig_pb_convert_vc_video_shortcode'          , array( &$this, 'convert_vc_video_shortcode'        ) );
		add_filter( 'ig_pb_convert_vc_gmaps_shortcode'          , array( &$this, 'convert_vc_gmaps_shortcode'        ) );
		add_filter( 'ig_pb_convert_vc_raw_html_shortcode'       , array( &$this, 'convert_vc_raw_html_shortcode'     ) );
		add_filter( 'ig_pb_convert_vc_raw_js_shortcode'         , array( &$this, 'convert_vc_raw_html_shortcode'     ) );
		add_filter( 'ig_pb_convert_vc_progress_bar_shortcode'   , array( &$this, 'convert_vc_progress_bar_shortcode' ) );
		add_filter( 'ig_pb_convert_vc_pie_shortcode'            , array( &$this, 'convert_vc_pie_shortcode'          ) );

		// Register action to finalize data conversion
		add_action( "ig_pb_after_convert_{$this->converter}_data", array( &$this, 'finalize' ) );
	}
	...
	public function prepare_visual_composer_data() {
		...
		// Clean-up all nested rows and columns because IG PageBuilder does not support nested row
		...
		// Wrap continuous `vc_toggle` shortcode inside an IG Accordion element
		...
		if ( 1 < count( $parts ) ) {
			...
			for ( $i = 1, $n = count( $parts ); $i < $n; $i++ ) {
				...
				if ( $first ) {
					$part = '[ig_accordion multi_open="yes"]' . $part;

					$first = false;
				}

				if ( '[/vc_toggle]' != substr( $part, -12 ) ) {
					$part = str_replace( '[/vc_toggle]', '[/vc_toggle][/ig_accordion]', $part );
		...
		foreach ( self::$widget_mapping as $tag => $widget ) {
			if ( false !== strpos( $this->post->post_content, '[' . $tag ) ) {
				$this->post->post_content = str_replace( '[' . $tag, '[ig_widget widget_id="' . $widget . '"', $this->post->post_content );
			}
		}
	...
	public function convert_vc_gallery_shortcode( $element ) {
		...
		for ( $i = 0, $n = count( $images ); $i < $n; $i++ ) {
			if ( 1 < $element['attributes']['slides_per_view'] || ( isset( $element['attributes']['onclick'] ) && 'link_no' != $element['attributes']['onclick'] ) ) {
				...
				if ( (int) $element['attributes']['slides_per_view'] == count( $slides ) || $n == $i + 1 ) {
					// Create new IG Item Carousel element
					$children[] = array(
						'tag'      => 'ig_item_carousel',
						'children' => $slides,
					);
				...
				// Create new `ig_item_carousel` shortcode
				$children[] = array(
					'tag'        => 'ig_item_carousel',
		...
		// Create new IG Carousel element
		$carousel = array(
			'tag'        => 'ig_carousel',
	...
	public function convert_vc_posts_grid_shortcode( $element ) {
		// Check if equivalent element available in IG PageBuilder
		global $shortcode_tags;

		if ( ! array_key_exists( 'ig_contentclips', $shortcode_tags ) ) {
		...
		if ( isset( $params['href'] ) && ! empty( $params['href'] ) ) {
			...
			if ( 'vc_button2' == $element['tag'] ) {
				...
				foreach ( $link_attrs as $link_attr ) {
					...
					if ( preg_match( '/^([a-z]+:)?//', $link_attr ) ) {
						$params['href'] = $link_attr;
					} else {
						$pair = explode( ':', $link_attr, 2 );

						if ( 'title' == $pair[0] ) {
							$params['el_title'] = $pair[1];
						} else {
	...
	public function convert_vc_progress_bar_shortcode( $element ) {
		...
		if ( isset( $params['values'] ) && ! empty( $params['values'] ) ) {
			...
			foreach ( $children as & $item ) {
				...
				// Create new `ig_item_progressbar` shortcode
				$item = array(
					'tag'        => 'ig_item_progressbar',
					'attributes' => array(
						'pbar_text'       => $text,
	...
	public function convert_vc_pie_shortcode( $element ) {
		...
		if ( ! array_key_exists( 'ig_progresscircle', $shortcode_tags ) ) {
			// Equivalent element not available, keep original shortcode as is
			return $element;
		}

		// Convert shortcode tag
		$element['tag'] = 'ig_progresscircle';
