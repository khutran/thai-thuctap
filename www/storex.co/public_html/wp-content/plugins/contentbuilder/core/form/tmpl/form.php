...
<form action="<?php echo esc_url( $this->action ); ?>" class="<?php esc_attr_e( empty( $alignment ) ? 'ig-form' : "ig-form form-{$alignment}" ); ?>" id="<?php esc_attr_e( $this->id ); ?>" method="POST" name="<?php esc_attr_e( $this->id ); ?>" role="form">
...
?>
			<div class="tab-pane<?php if ($first) echo ' active'; ?>" id="ig-form-section-<?php esc_attr_e( $sid ); ?>"<?php if ( isset( $section['attributes'] ) ) echo ' ' . implode( ' ', $section['attributes'] ); ?>>
<?php
endif;
...
$script = '
		// Init form
		new $.IG_Form(' . json_encode( array_merge( array( 'form_id' => esc_attr( $this->id ) ), array_combine( $js_init, array_map( 'is_string', $js_init ) ) ) ) . ');';
