...
if ( ! class_exists( 'IG_Carousel' ) ) {
	...
	class IG_Carousel extends IG_Pb_Shortcode_Parent {
		...
		public function element_config() {
			...
			$this->config['name']             = esc_html__( 'PT Carousel',  'storex' );
			$this->config['has_subshortcode'] = 'IG_Item_' . str_replace( 'IG_', '', __CLASS__ );
            $this->config['edit_using_ajax'] = true;
            $this->config['exception'] = array(
				'default_content'  => esc_html__( 'PT Carousel',  'storex' ),
				'data-modal-title' => esc_html__( 'PT Carousel',  'storex' ),
		...
		public function element_items() {
			$this->items = array(
				...
				'styling' => array(
					...
					array(
                        'name'    => esc_html__( 'Padding items',  'storex' ),
                        'id'      => 'padding_items',
                        'type'    => 'select',
                        'std'     => 'item-indent',
                        'options' => array(
							'item-indent' => 'With padding',
							'no-indent' => 'Without padding',
						),
                    ),
					array(
                        'name'    => esc_html__( 'Border items',  'storex' ),
                        'id'      => 'border_items',
                        'type'    => 'select',
                        'std'     => '',
                        'options' => array(
							'item-border' => 'With border',
							'' => 'Without border',
						),
                    ),
					array(
		...
		public function element_shortcode_full( $atts = null, $content = null ) {
			...
			$html_output .= "<div class='title-wrapper'><h2>{$el_title}</h2>";
			if ( $show_arrows == 'yes' ) { $html_output .= "<div class='slider-navi'><span class='prev'><i class='fa fa-angle-left'></i></span><span class='next'><i class='fa fa-angle-right'></i></span></div>"; }
			$html_output .= "</div><div class='carousel-container {$hover_effect} per-slide-{$per_slide} {$padding_items} {$border_items}' {$styles}>";
			$html_output .= $carousel_content;
			$html_output .= "</div></div>";

			if($owlTransition!=="false"){
				$owlTransition='"'.$owlTransition.'"';
			}
