...
function pt_gallery( $blank = NULL, $attr ) {
    ...
    $gallery_div = "<div id='$selector' data-isotope=container data-isotope-layout=masonry data-isotope-elements=gallery-item class='pt-gallery gallery columns-{$columns} galleryid-{$id}'>";
    ...
    foreach ( $attachments as $id => $attachment ) {
        ...
        if ( $captiontag && trim($attachment->post_title) ) {
            $output .= "
                ...
                <div class='wrapper-buttons-gallery'>
                    <a class='quick-view' rel='nofollow' href='".$attachment->guid."' title='".esc_html__('Quick View', 'storex')."'><i class='fa fa-search'></i></a>
                    <a class='link-to-post' rel='bookmark' href='".get_permalink($attachment->ID)."' title='".esc_html__( 'Click to learn more', 'storex')."'><i class='fa fa-bars'></i></a>
                </div> 
                <div class='wrapper-info'><h3>" . wptexturize($attachment->post_title) . "</h3>";
                
            $output .= "</div><div class='vertical-helper'></div></{$captiontag}>";
        }
        $output .= "</{$itemtag}>";
...
<?php /* Check if site turned to boxed version */
	  ...
	  $gallery_title = esc_html__('Gallery', 'storex');
	  $rb_color = pt_get_post_pageribbon($post->ID);
?>
	<div class="header-stripe" style="background-color:<?php echo $rb_color; ?>">
		<?php if (!$boxed || $boxed=='') : ?><div class="container"><?php endif; ?>
			<div class="row">
				...
				<div class="col-md-4 col-sm-4 col-sx-12">
					<h1 class="title"><?php echo esc_attr($gallery_title); ?></h1>
				</div>
